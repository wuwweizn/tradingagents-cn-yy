    ...
    with st.form("analysis_form", clear_on_submit=False):
        ...
        with col2:
            ...
            try:
                ...
                # 获取当前选择的模型点数和开关状态
                try:
                    from utils.model_points import get_model_points, get_points_toggle_config, get_analysis_points
                    ...
                    if parts:
                        ...
                        help_text = f"选择分析的深度级别，级别越高分析越详细但耗时更长\n当前选择：{points_detail} = {total_points}点"
                    else:
                        help_text = f"选择分析的深度级别，级别越高分析越详细但耗时更长\n当前选择：{cached_depth}级（点数消耗功能已关闭）"
                except Exception:
                    help_text = f"选择分析的深度级别，级别越高分析越详细但耗时更长\n当前选择：{cached_depth}级，基础消耗 {depth_points} 点"
            ...
            try:
                ...
                llm_provider = st.session_state.get('llm_provider', 'dashscope')
                llm_model = st.session_state.get('llm_model', 'qwen-turbo')
                total_points = get_analysis_points(research_depth, llm_provider, llm_model)
                ...
                if parts:
                    ...
                    if total_points > 0:
                        st.caption(f"💡 预计消耗: {total_points} 点（{points_info}）")
                    else:
                ...
                try:
                    ...
                    st.caption(f"💡 研究深度基础消耗: {current_points} 点")
                except Exception:
                    pass
